...
Every visual asset is produced one of two ways: **authored in code** (inline SVG or CSS) or **generated as an image**. Decide per asset, never once for the whole project — mixing the two is the intent, not a compromise.

### The test

> Can you name every shape in it? Author it in code.
> Do you have to describe a texture, a species, or a lighting condition? Generate an image.

### Author in code — the default

- **Use when** the asset reduces to flat geometry: four flat colors or fewer, no gradient or real texture required, and it stays recognizable when simplified onto a 16 × 16 grid.
...
- **Use when** code would produce a bad likeness: organic detail, many colors, gradients, real texture, fine curves, or any mark whose identity dies on simplification.
- **Fits**: mascots and creature marks, photographic or atmospheric imagery, illustration, portraits, anything painterly.
- **Rules that follow from the runtime**:
  - Embed it. The page's CSP blocks every external image host, so an asset is either an inline base64 entry or an uploaded asset referenced relatively — never a remote URL.
  - Budget roughly 70 KB per image; the whole document republishes on every save.
  - Ship one variant per surface when it appears on both light and dark tiles. A raster asset cannot recolor itself.
  - An image resting on a surface takes the system product-shadow (`rgba(0, 0, 0, 0.22) 3px 5px 30px`) — the single shadow in the system. Chrome still takes none.
  - Generated images survive PNG/PDF export unchanged, which webfonts do not. Where an export path matters, an image is the more reliable carrier than styled text.

### When neither is honest

If code cannot produce a credible likeness **and** generating one would be a guess at somebody's trademark, use a neutral marker and name it in the handover. A visible placeholder is a smaller error than a wrong logo, and it tells the reader exactly what still needs a decision.

Third-party technology marks on a portfolio or CV are nominative use — naming what you work with. Draw or generate a rendition, keep the wordmark beside it so identification never rests on the glyph alone, and do not repackage a vendor's official asset files.
...
- Produce each asset by the **Assets** test — flat geometry authored in code, organic or textured artwork as a generated image — and mix both freely within one surface.
...
- Don't force a complex mark into hand-drawn code to keep the asset in-repo — a bad likeness costs more credibility than an embedded image costs bytes. And don't reach for a generated image for something that is four rectangles.
